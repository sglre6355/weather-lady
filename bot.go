...
func (b *WeatherBot) handleSubscribeWeather(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	err = b.AddSubscription(channelID, guildID, parsedTime)
	if err != nil {
		if err := s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
			Type: discordgo.InteractionResponseChannelMessageWithSource,
			Data: &discordgo.InteractionResponseData{
	...
	if err := s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseChannelMessageWithSource,
		Data: &discordgo.InteractionResponseData{
			Content: fmt.Sprintf("Successfully subscribed this channel to receive weather forecasts at %s daily", timeStr),
		},
	}); err != nil {
		log.Printf("Error responding to interaction: %v", err)
	}
...
func (b *WeatherBot) AddSubscription(channelID, guildID string, scheduledTime time.Time) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	go b.scheduleWeatherUpdate(channelID, nextRun, stopChan)
...
func (b *WeatherBot) scheduleWeatherUpdate(channelID string, nextRun time.Time, stopChan chan struct{}) {
	...
	for {
		select {
		case <-timer.C:
			b.sendWeatherForecast(channelID)
